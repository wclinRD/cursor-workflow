...
├── commands/                      think, plan, review, status, compact
├── hooks/
...
## Token 策略（Smart MCP + Hooks 自動化）

| 機制 | 自動？ | 說明 |
|------|--------|------|
| **caveman** | ✅ Hook | `preToolUse` 替 exa search/crawl 注入壓縮 |
| **smart_compact** | ✅ 半自動 | `postToolUse` 每 15 次 tool 注入指示，Agent 執行 compact |
| **`/compact`** | 可選 | 僅手動提前 compact |

Hook 實作：`.cursor/hooks/token-automation.mjs`  
詳見 `.cursor/rules/smart-mcp.mdc`。
